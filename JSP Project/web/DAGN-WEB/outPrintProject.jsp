...
<%
    ...
    String filepath = String.format("D:/githubOutPut/Project/%s.xls",username);
...
<script language="JavaScript">
    ...
    var sameForkCount = new Array();
    <%
    for(int i=1;i<=rows;i++){
    %>
    userName[<%=i-1%>] = "<%=defaultSheet.getCell(0,i).getContents()%>";
    sameForkCount[<%=i-1%>] = <%=defaultSheet.getCell(1,i).getContents()%>;
    <%}
    %>
    window.onload = function () {
        ...
        for(var i=0;i<Number(<%=rows %>);i++)
        {
            ...
            var node2 = document.createTextNode("共同Fork:"+sameForkCount[i]);
            ...
            img.src = "imgs/project/"+(random%66)+".jpg";
